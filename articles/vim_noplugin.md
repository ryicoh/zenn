...
## フォルダ内置換は `:cfdo %s/prev/next/g | w` を使う

`:cfdo` はquickfix windowのファイルごとにコマンドを実行するコマンドである。

置換前の文字列を `:vim` で検索してから、`:cfdo %s/<C-R><C-W>/new text/g | w` とすることで、%sの置換を実行できる
`|` は続けて後ろのコマンドを実行でき、`w`で保存する。
...
* タグジャンプの`<C-]>`が使えます。ウィンドウ分割する場合は`<C-w>]`です。
